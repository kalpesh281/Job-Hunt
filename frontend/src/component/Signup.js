...
const Signup = (props) => {
  ...
  const responseGoogle = (response) => {
    if (response && response.profileObj && response.profileObj.email) {
      ...
      const refreshToken = response.googleId; // Assuming googleId is used for refreshToken
      axios.post(apiList.googleSign, { email, accessToken, refreshToken })
        .then((response) => {
          // Handle successful signup or login response
    ...
    <Paper elevation={3} className={classes.body}>
      <Grid container direction="column" spacing={4} alignItems="center">
        ...
        <Grid item>
          ...
          {/* <Grid item>
              <GoogleLogin
                clientId="900799958897-09pkgcv3qiimpuabeifppv06q4v4fjoe.apps.googleusercontent.com"
                buttonText="Signup with Google"
                onSuccess={responseGoogle}
                onFailure={(error) => console.error("Google Login Error:", error)}
                cookiePolicy={'single_host_origin'}
                redirectUri="http://localhost:3000/auth/signup/google" // Adjust the port if necessary
              />
          </Grid> */}
        </Grid>
      </Grid>
